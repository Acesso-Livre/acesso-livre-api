...
from acesso_livre_api.src.locations import schemas, service
...
@router.post("/accessibility-items/", response_model=schemas.AccessibilityItemResponse)
def create_accessibility_item(item: schemas.AccessibilityItemCreate, db: Session = Depends(get_db)):
...
@router.get("/accessibility-items/{item_id}", response_model=schemas.AccessibilityItemResponse)
def get_accessibility_item_by_id(item_id: int = Path(..., gt=0), db: Session = Depends(get_db)):
...
@router.patch("/{location_id}", response_model=schemas.LocationDetailResponse)
def update_location(location_update: schemas.LocationUpdate, location_id: int = Path(..., gt=0), db: Session = Depends(get_db)):
    location = service.update_location(db=db, location_id=location_id, location_update=location_update)
    return location

@router.delete("/{location_id}", response_model=schemas.LocationDeleteResponse)
def delete_location(location_id: int = Path(..., gt=0), db: Session = Depends(get_db)):
