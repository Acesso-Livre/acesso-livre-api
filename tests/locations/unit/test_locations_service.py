...
async def test_update_location_average_rating_first_comment():
    ...
    result = await update_location_average_rating(mock_db, location_id=1, new_value=5.0)
...
async def test_update_location_average_rating_location_not_found():
    ...
    result = await update_location_average_rating(mock_db, location_id=999, new_value=5.0)
...
async def test_update_location_average_rating_handling_none_avg():
    ...
    mock_db.query().filter().first.return_value = mock_location
    mock_db.query().filter().count.return_value = 1
    result = await update_location_average_rating(mock_db, location_id=1, new_value=3.0)
